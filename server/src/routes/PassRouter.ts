...
router.post('/purchase', async (req: Request, res: Response): Promise<any> => {
    ...
    try {
        ...
        if (passData) {
            ...
            PassModel.update({ num_available: numAvailable, num_purchased: numPurchased }, 
                { where: { id: passId } }
                );
        }
...
router.delete('/:id', async (req: Request, res: Response): Promise<any> => {
    try {
        await PassModel.destroy({ where: { id: req.params.id } });
        sendResponse('Successfully deleted pass.', 200, res);
    } catch (err) {
        console.log(err);
