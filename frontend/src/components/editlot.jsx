...
class EditLot extends React.Component {
  ...
  async getLot(id) {
    ...
    let response = await fetch(
      "http://localhost:5000/lot/get/" + id,
      options
    ).then((res) => res.json());
  ...
  getPass = async (id) => {
    ...
    let response = await fetch(
      "http://localhost:5000/pass/passById/" + id,
      options
    ).then((res) => res.json());
  ...
  handleSubmit = async () => {
    ...
    let response = await fetch(
      "http://localhost:5000/pass/passByName/" + this.state.pass,
      options
    ).then((res) => res.json());
    ...
    response = await fetch("http://localhost:5000/lot/", options).then((res) =>
      res.json()
    );
  ...
  render() {
    ...
    return (
      ...
      >
        <h2>Select the pass you would like to register a new parking lot to</h2>
        <Complete />
        <Divider />
        ...
        <Form.Item label="Allowable Pass Level">
          <InputNumber
            value={this.state.passlevel}
            onChange={(value) => {
              this.setState({ passlevel: value });
            }}
          />
        </Form.Item>
