...
class UserVehicles extends React.Component {
  ...
  handleRemove = async (id) => {
    ...
    await fetch("http://localhost:5000/car/" + id, options).then((res) =>
      res.json()
    );
    this.fetchCars();
  ...
  handleSubmit = async () => {
    ...
    response = await fetch("http://localhost:5000/car/", options).then((res) =>
      res.json()
    );
  ...
  fetchCars = async () => {
    ...
    response = await fetch(
      "http://localhost:5000/car/" + response.info.id,
      options
    ).then((res) => res.json());
  ...
  render() {
    ...
    return (
      <>
        <Divider orientation="left">Registered Vehicles</Divider>
        <Table dataSource={this.state.filteredData} columns={columns} />
        <Divider orientation="left">Registere a new vehicle</Divider>
        <Form {...formItemLayout} layout="horizontal">
          <Form.Item label="Make">
            <Input
              onChange={(e) => (this.make = e.target.value)}
          ...
          <Form.Item label="License Plate">
            <Input
              onChange={(e) => (this.plate = e.target.value)}
              placeholder="Registration of Vehicle"
            />
          </Form.Item>
